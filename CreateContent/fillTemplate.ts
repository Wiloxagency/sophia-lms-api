...
function processContentWithConstraints(
  ...
  if (words.length > constraints.max) {
    return sliceText(
      content,
      content.split(" ", constraints.max + 1).join(" ").length,
      isTitle
    );
  }
...
function replaceTemplateColors(
  template: any[][],
...
export async function fillTemplate(
  ...
  try {
    ...
    let processedSlides = slides.map((slide: any) => {
      ...
      const template = GlassTemplate.find(
        (t: any) => t[0].component === "meta-tag" && t[0].code === templateCode
      );
      ...
      return processedTemplate.map((component: any) => {
        ...
        if (component.component === "audio") {
          component.audioUrl =
            slide.isAudioExternal === true
              ? slide.externalAudioUrl
              : slide.audioUrl;
          return component;
        }
        ...
        // };
        const mediaTypes = {
          "video-h": (assetType: string, orientation: string) =>
            assetType == "video",
          "video-v": (assetType: string, orientation: string) =>
            assetType == "video",
          "image-h": (assetType: string, orientation: string) =>
            assetType == "photo",
          "image-v": (assetType: string, orientation: string) =>
            assetType == "photo",
          "image-q": (assetType: string, orientation: string) =>
            assetType == "photo",
          "icon-b": (assetType: string) => assetType == "icon",
          "icon-w": (assetType: string) => assetType == "icon",
        };
        ...
        if (component.video) {
          ...
          const matchingAssetIndex = slide.assets.findIndex(
            (asset: any) =>
              asset.assetType === "video" &&
              mediaTypes[videoType](asset.assetType, asset.orientation)
          );
        ...
        if (component.image) {
          ...
          const matchingAssetIndex = slide.assets.findIndex(
            (asset: any) =>
              ...
              mediaTypes[imageType](asset.assetType, asset.orientation)
          );
        ...
        if (component.icon) {
          ...
          const matchingAssetIndex = slide.assets.findIndex(
            (asset: any) =>
              ...
              mediaTypes[iconType](asset.assetType)
          );
        ...
        if (
          component.title &&
          component.title.startsWith("[") &&
          component.title.endsWith("]")
        ) {
          if (component.title === "[title]") {
            ...
            const sectionContent =
              slide.slideContent.sections[sectionIndex]?.subtitle || "";
        ...
        if (
          ...
          if (component.text === "[text]") {
            ...
            const sectionContent =
              slide.slideContent.sections[sectionIndex]?.text || "";
            if (constraints.sections && constraints.sections[sectionIndex]) {
              component.text = processContentWithConstraints(
                sectionContent,
    ...
    processedSlides = replaceTemplateColors(
      processedSlides,
      globalData.defaultTheme.colors
    );
    ...
    const containerClient =
      blobServiceClient.getContainerClient("presentations");
    const blobClient = containerClient.getBlockBlobClient(
      `${presentationName}.json`
    );

    await blobClient.upload(
      JSON.stringify(processedSlides),
      JSON.stringify(processedSlides).length
    );

    return {
      status: "success",
      message: `Presentation ${presentationName}.json created successfully`,
    };
  } catch (error) {
