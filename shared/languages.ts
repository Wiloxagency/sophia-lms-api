...
export const languagesArray = Object.entries(languagesObject).map(
...
export type LanguageCode = keyof typeof languagesObject;
export type LanguageName = ObjectValue<typeof languagesObject>["language"];

export async function returnLanguageAndLocaleFromLanguage(
  languageCode: string
) {
...
export const detectLanguage = async (phrase: string): Promise<string> => {
  try {
    const prompt = `Detect the language of this phrase: "${phrase}".
