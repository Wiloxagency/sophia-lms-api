...
export async function saveCourseCreationLog(
  courseCode: string,
  courseTitle: string,
  ...
  const logPayload = {
    ...
    timestamp: date,
    isSelfManageable: isSelfManageable,
    userCode: userCode,
  };
  try {
    const db = await database;
...
export async function deleteCourseCreationLog(
  courseCode: string,
  sections: any
) {
  try {
      ...
      section.elements.forEach((element: any) => {
        if (element.type == "Lección Engine") {
          let paragraphIndex = compareObjectStructures(
            element.elementLesson.paragraphs
          );
          if (paragraphIndex >= 0) {
            complete = false;
          }
    ...
    if (complete) {
      const db = await database;
      const CoursesUnderConstruction = db.collection(
        "coursesUnderConstruction"
      );
      const DeleteLog = await CoursesUnderConstruction.deleteOne({
        courseCode: courseCode,
      });
    }
  } catch (error) {
