      ...
      for (const paragraph of element.elementLesson?.paragraphs || []) {
        ...
        if (paragraph.imageData?.finalImage?.url) {
          const uploadedImagePath = await uploadFileFromUrl(
            containerClient,
            assetsFolder,
            paragraph.imageData.finalImage.url
          );
        ...
        if (paragraph.videoData?.finalVideo?.url) {
          const uploadedVideoPath = await uploadFileFromUrl(
            containerClient,
            assetsFolder,
            paragraph.videoData.finalVideo.url
          );
...
const httpTrigger: AzureFunction = async function (
  ...
  if (course.isNewSlideStructure === true) {
    ...
    downloadLink = await createScormV2(
      course,
      selectedElements,
      userEmail,
      userName
    ...
    downloadLink = await createScorm(
      context,
      course,
      selectedElements,
      userEmail,
