...
const httpTrigger: AzureFunction = async function (
  ...
  const createStudent = async () => {
    ...
    try {
      ...
      const check = Students.findOne({ email: req.body.email });
      var body = null;
      body = await check;
      if (body) {
        context.res = {
        ...
        const resp = Students.insertOne(newUser);
        body = await resp;
        if (body.acknowledged) {
          const user = await Students.findOne({ _id: body.insertedId });
      ...
      return context.res = {
        ...
        statusText: "Create student error",
      };
    }
  };
  ...
  const deleteStudent = async () => {
    try {
      ...
      const resp = Students.deleteOne({ email: req.headers.email });
      ...
      context.res = {
        ...
        body: {
          message: "Error deleting student by email",
        },
      };
    }
  };
  ...
  const updateStudent = async () => {
    try {
      const db = await database;
      const Students = db.collection("student");
      const resp = Students.findOneAndUpdate(
      ...
      if (body.value != null) {
        ...
        context.res = {
          ...
          body: {
            message: "Error updating student by email",
          },
        };
      }
    } catch (error) {
